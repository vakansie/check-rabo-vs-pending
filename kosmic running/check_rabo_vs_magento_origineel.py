...
ignore_transfers_from = ('STICHTING PAY.NL', 'TEN HAG ASS ADV BV', 'DE AMERSFOORTSE VERZ', 'ADP LOGISTICS', 'AAB RETAIL INZ TIKKIE', '')
...
def get_rabo_and_magento_file(download_path):
    ...
    while files_found == False: 
        list_of_rabo_files = glob.glob(download_path+'\\CSV_T_*.csv') 
...
def process_the_rabo_file(rabo_file):
    rabo_payment_list = []
        ...
        for row in reader:
            ...
            try:
                ...
                try:
                    ...
                    date = datetime.datetime.strptime(transaction[4], '%Y-%m-%d')
...
def weigh(rabo_payment_list, magento_order_list):
    for payment in rabo_payment_list:
        ...
        # ### points for newest of similar orders
        # for match_1 in weight_matched_order_list:
        #     print(match_1)
        #     for match_2 in weight_matched_order_list:
        #         if (weight_matched_order_list.index(match_1) != weight_matched_order_list.index(match_2)) and (match_1.name == match_2.name):
        #             match_1.match_weight += 5 * (match_1.date > match_2.date)
...
def check_rabo_file_vs_magento_file(download_path):
    ...
    rabo_payment_list = process_the_rabo_file(rabo_file)
