...
class Order:
    def __init__(self, order_number, name, amount_owed, date, order_status, email, payment_method, shipping_method, country):
        self.order_number     = order_number
        self.name             = name
...
class Payment_Order_Match:
    def __init__(self, order_number, amount_owed, amount_paid, name, amount_check_result, email, shipping_method, country):
        self.order_number           = order_number
        self.amount_owed            = amount_owed
        self.amount_paid            = amount_paid
        self.name                   = name
        ...
        self.email                  = email
        self.shipment_dict          = {'Tracking service - Express delivery: Monday till Friday':'EXPRESS SHIPPING', 'Letter mail - Letter mail <No tracking>':'LETTER MAIL', 'Come to our store in Enschede - Pick Up': 'PICK UP'}
        self.shipping_method        = self.shipment_dict[shipping_method] if shipping_method in self.shipment_dict else 'normal shipping'
        self.country                = country
        if self.country not in parcel_countries and self.shipping_method == 'normal shipping': self.shipping_method = 'GLS PLENTY PARTS'
    ...
    def __repr__(self):
        ...
        return f'{self.order_number:<13}-{formatted_amount_owed:>9}  -{formatted_amount_paid:>9}  -  {self.name:<28} - {self.amount_check_result} - {self.shipping_method:>20} - {self.country:>15}'
    ...
    with open(magento_file, 'rt', encoding = "utf-8", errors='ignore' ) as p:
        ...
        for row in reader2:
            ...
            if row_contains_order == True:
                order = Order(order_number, name, amount_owed, date, order_status, email, payment_method, shipping_method, country)
...
def get_rabo_date_format(rabo_file):
    ...
    if 'CSV_T_'              in file_name: rabo_date_format = '%Y-%m-%d'
    if 'ZOEK_RESULTAAT_'     in file_name: rabo_date_format = '%d/%m/%Y'
    if 'TRANSACTION_SEARCH_' in file_name: rabo_date_format = '%d-%m-%Y'
...
def process_the_rabo_file(rabo_file, date_format):
    ...
    with open(rabo_file, 'rt', encoding = "ISO-8859-1") as f:
        ...
        for row in reader:
            ...
            if transaction[9].upper() in ignore_transfers_from:
                continue
            else:
                try:
...
def weigh(rabo_payment_list, magento_order_list):
    for payment in rabo_payment_list:
        ...
        for order in magento_order_list:
            ...
            percentage_of_amount_owed = (paid - owed) / owed if owed else 0
        ...
        for order in magento_order_list:
            ...
            if payment_matched_to_order(payment, order):
                ...
                and all(order.order_number != already_matched.order_number for already_matched in matched_payments_list):
                        matched_payment = Payment_Order_Match(order.order_number, order.amount_owed, payment.amount_paid, order.name, 'not checked', order.email, order.shipping_method, order.country) 
...
def get_pay_at_the_store_orders(magento_order_list):
    ...
    for order in magento_order_list:
        if order.payment_method == 'checkmo' and order.order_status.lower() in ['pending', 'processing', 'holded']:
            print(order)
