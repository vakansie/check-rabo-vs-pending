...
class UI:
    ...
    def generate_main_ui_elements(self):
        ...
        self.import_button = tk.Button(self.window, text='Add Products From SQL File', command=lambda: add_from_file()).grid(row=len(self.product_dict)+2,column=2)
...
def get_SQL_file(SQL_file_path):
        ...
        try:
            ...
            ui.file_field.insert(tk.END,f'ERROR: no SQL file found in {SQL_file_path}')
            return
